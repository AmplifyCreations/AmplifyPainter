...
Tablet::Tablet()
{
	...
	// Get cursor num
	Wintab::WTInfo( WTI_INTERFACE, IFC_NCURSORS, &cursorNum_);
...
void Tablet::Open(HWND hwnd)
{
	...
	context.lcOptions |= CXO_SYSTEM | CXO_MESSAGES;// CXO_MESSAGES;
...
{
	// NOTE: we may need to process the packets in proximity change

	if( !LOWORD( lp ) )
	{
		//PACKET proximityBuffer[ 1 ];
		//const int totalPacks = Wintab::WTPacketsGet( context_, 1, proximityBuffer );
		//if( totalPacks )
		//{
		//	UINT pkCursor = proximityBuffer[ 0 ].pkCursor;
		//	Debug::Log( pkCursor );
		//}
		//Debug::Log( totalPacks );
	...
	else
	{
		...
		//	UINT cursorType;
		//	Wintab::WTInfo( WTI_CURSORS + pkCursor, CSR_TYPE, &cursorType );
		//	CursorType t = static_cast<CursorType>( pkCursor % static_cast<int>( CursorType::Num ) );
		//	Debug::Log( static_cast<int>(t) );
		//	Debug::Log( ( INT64( cursorType & 0xFF6 ) << 32L ) );
		//}
		//Debug::Log( totalPacks );
		proximity_ = TRUE;
	}
...
UINT Tablet::GetCursorNum() const
{
	return cursorNum_;
}
